extends "res://addons/SphynxDOFToolkit/DOF/base_dof.gd"
class_name DOFCompositorEffect

@export_group("Shader Stages")
@export var dof_stage : ShaderStageResource = preload("res://addons/SphynxDOFToolkit/DOF/dof_stage.tres"):
	set(value):
		unsubscribe_shader_stage(dof_stage)
		dof_stage = value
		subscirbe_shader_stage(value)

@export var overlay_stage : ShaderStageResource = preload("res://addons/SphynxDOFToolkit/DOF/dof_overlay_stage.tres"):
	set(value):
		unsubscribe_shader_stage(overlay_stage)
		overlay_stage = value
		subscirbe_shader_stage(value)

@export var tile_max_x_stage : ShaderStageResource = preload("res://addons/SphynxDOFToolkit/DOF/dof_tile_max_x_stage.tres"):
	set(value):
		unsubscribe_shader_stage(tile_max_x_stage)
		tile_max_x_stage = value
		subscirbe_shader_stage(value)

@export var tile_max_y_stage : ShaderStageResource = preload("res://addons/SphynxDOFToolkit/DOF/dof_tile_max_y_stage.tres"):
	set(value):
		unsubscribe_shader_stage(tile_max_y_stage)
		tile_max_y_stage = value
		subscirbe_shader_stage(value)

@export var neighbor_max_stage : ShaderStageResource = preload("res://addons/SphynxDOFToolkit/DOF/dof_neighbor_max_stage.tres"):
	set(value):
...
func _render_callback_2(render_size : Vector2i, render_scene_buffers : RenderSceneBuffersRD, render_scene_data : RenderSceneDataRD):
	...
	ensure_texture(tile_max_x, render_scene_buffers, RenderingDevice.DATA_FORMAT_R32_SFLOAT, Vector2(1. / tile_size, 1.))
	ensure_texture(tile_max, render_scene_buffers, RenderingDevice.DATA_FORMAT_R32_SFLOAT, Vector2(1. / tile_size, 1. / tile_size))
	ensure_texture(neighbor_max, render_scene_buffers, RenderingDevice.DATA_FORMAT_R32_SFLOAT, Vector2(1. / tile_size, 1. / tile_size))
	...
	for view in range(view_count):
		...
		[
			get_sampler_uniform(custom_depth_image, 0, false),
		...
		dispatch_stage(dof_stage, 
		[
			get_sampler_uniform(color_image, 0, false),
			get_sampler_uniform(custom_depth_image, 1, false),
			get_sampler_uniform(neighbor_max_image, 2, false),
			get_image_uniform(output_color_image, 3),
		],
